...
trait FoodDao extends FoodPersistence {
  this: DBModule =>

  def create(foodItem: FoodItem): Future[Long] = db.run(foodTableAutoInc += foodItem)

  def update(foodItem: FoodItem): Future[Long] = db.run(foodTableQuery.filter(_.id === foodItem.id).update(foodItem)).mapTo[Long]
...
final case class FoodItem(name: String, id: Long = 0)
